...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "CorbaCodeShare" }, status: "idle" });
...
//--------------------------GÜVENLİ LİSTE--------------------------\\
  if(command === "güvenli") {
    ...
    .setFooter("CorbaCodeShare - Guard Botu")
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "CorbaCodeShare - Guard Koruma Sistemi" }).catch();
};
//--------------------------CEZALANDIRMA--------------------------\\
...
.setFooter(`CorbaCodeShare - v12 Guard Botu`))
} else { 
...
.setDescription(`${entry.executor} İsimli Yetkili ${member} İsimli Kullanıcıyı Sağ Tık Kullanarak Sunucudan Kickledi Gerekli İşlemler Yapıldı`)
.addField("Yapılan İşlem",`${entry.executor} İsimli Kullanıcı Jail'e Atıldı`)
.setFooter(`CorbaCodeShare - v12 Guard Botu`)).catch(err => {}); };
});
//--------------------------SAĞ TIK KİCK KORUMASI--------------------------\\
...
.addField("Yapılan İşlem",`${entry.executor} İsimli Kullanıcı Jail'e Atıldı`)
.setFooter(`CorbaCodeShare - v12 Guard Botu`)).catch(); 
} else { 
...
client.on("channelCreate", async channel => {
  ...
  channel.delete({reason: "CorbaCodeShare Kanal Koruma"});
...
client.on("channelDelete", async channel => {
  ...
  if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.channelGuard) return;
  cezalandir(entry.executor.id, "ban");
  await channel.clone({ reason: "CorbaCodeShare Kanal Koruma" }).then(async kanal => {
